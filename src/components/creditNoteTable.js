...
function CreditNoteTable() {
    const { creditsNote, loading, invoiceSelected, setSelectedCreditNote } = useInvoice();

    if (!invoiceSelected) {
        return (<></>)
    }
    
    if (invoiceSelected && loading) {
        return (
            <div>
    ...
    return (
        <div>
            ...
            <ul class="w-48 text-sm font-medium text-gray-900 bg-white rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                {creditsNote.map(({id, organization_id, amount, currency}, index) => 
                    (
